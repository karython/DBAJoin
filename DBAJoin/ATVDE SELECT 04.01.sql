...
select clientes.clienteid, clientes.nome, pedidos.pedidoid  from pedidos 
join clientes
on pedidos.clienteid = clientes.clienteid
join itenspedido
...
/*fazer a media dos pedidos do mes 10 e tambem o minimo*/

SELECT 
ROUND(avg(Valortotal),2)as ValorMedia,
min(ValorTotal)as ValorMin from pedidos where month(datapedido)= 10;
