...
INSERT INTO ItensPedido (PedidoID, ProdutoID, Quantidade) VALUES
    ...
    /*atividade 5*/
    select pedidos.PedidoID, pedidos.ValorTotal, pedidos.DataPedido, pedidos.ClienteID , nome
    from pedidos
    inner join Clientes 
    on pedidos.ClienteID = Clientes.ClienteID;
    /*ok*/
    ...
    from clientes
    left join pedidos on clientes.clienteid = pedidos.pedidoid
    ...
    add foreign key (TipoClienteID) 
    references TipoCliente (TipoID);
    
    /*Q3*/
    insert into TipoCliente (Tipo) values ('pessoa fisica'),('pessoa juridica');
    
    /*Q4*/
    update Clientes set TipoClienteID = 1 where Nome like '%A';
    update Clientes set TipoClienteID = 1 where Nome like '%0';
    
	/*Q5*/
    select distinct Nome, TipoCliente.Tipo 
    from Clientes left join TipoCliente on Clientes.TipoClienteID = TipoCliente.TipoID;
    
    /*Q6*/
    select Clientes.Nome, ifnull(TipoCliente.Tipo, 'Não foi especificado') as Tipo
    from Clientes left join TipoCliente on Clientes.TipoClienteID = TipoCliente.TipoID;
    
    /*Q7*/
    select Clientes.Nome from Clientes where TipoClienteID is null;
    
    /*Q8*/
    select Clientes.ClienteID, Clientes.Nome, TipoCliente.Tipo as Cliente
    from Clientes left join TipoCliente on Clientes.TipoClienteID = TipoCliente.TipoID;
    
    /*Q10*/
    select * from Pedidos where DataPedido between '2023-10-01' and '2023-10-31';
    ...
    group by Clientes.ClienteID, Pedidos.DataPedido 
    having count(*) > 1;
    
 
    
    /*Q13*/
    select Clientes.Nome, sum(Pedidos.ValorTotal) as TotalVendas from Clientes 
    join Pedidos on Clientes.ClienteID = Pedidos.ClienteID group by Clientes.Nome;
    
    /*Q14*/
    select Produtos.NomeProduto, count(*) as QuantidadeComprada from Produtos join Itenspedidos on Produtos.ProdutoID = ItensPedidos.ProdutoID
    group by Produtos.NomeProduto having count(*) > 1;
    
    /*Q15*/
    select Clientes.Nome, count(*) as QuantidadeComprada from Clientes 
    join Pedidos on Clientes.ClienteID = Pedidos.ClienteID
