...
select max(preco), min(preco), avg(preco) as Media from  livros;
...
select max(Quantidade), min(quantidade) from itenspedidos;
...
ValorTotal *1.1 as ValorTotal_Com_Aumento
from pedidos;
...
select PedidoID, DataPedido, ValorTotal,
ValorTotal*.1 as Aumento, 
ValorTotal*1.1 as ValorTotal_Com_Aumento 
from pedidos;

select PedidoID, DataPedido, ValorTotal,
ValorTotal/10 as Aumento, 
ValorTotal*1.1 as ValorTotal_Com_Aumento 
from pedidos;

/*4*/

select upper(NomeCliente)  as NomeCliente, 
lower(Email) as Email
from clientes;

/*5*/

select substring(NomeCliente,1,3) as NomeCliente, Email 
 from clientes;

/*6*/

select *, current_date() - DataPedido  as Diferenca_Dias
 from pedidos;

/*7*/
select *, datediff(current_Date,DataPedido) as Diferenca_dias,
 date_add(datapedido,interval "15" day )as Acrescimo_de_dias
 from pedidos ;
...
select *, datediff(current_Date,datapedido) as Diferenca_dias,
 date_sub(datapedido,interval "15" day )as Acrescimo_de_dias 
 from pedidos ;
