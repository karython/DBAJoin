...
select * from produtos order by preco > 10 desc limit 0,5;

select * from pedidos where valorTotal  between 120.00 and 190.00 order by valorTotal desc;

SELECT pedidos.pedidoid , clientes.nome
from pedidos 
join clientes on pedidos.clienteid = clientes.clienteid;

select pedidos.pedidoid,pedidos.DataPedido,pedidos.ValorTotal,clientes.ClienteID,clientes.Nome
from pedidos
inner join clientes on pedidos.ClienteID = Clientes.ClienteID;

select pedidos.valortotal,clientes.clienteid
from pedidos
inner join clientes on pedidos.clienteid = clientes.clienteid where valortotal between 200 and 270; 

select pedidos.valortotal,clientes.clienteid,clientes.nome
from pedidos
inner join clientes on pedidos.clienteid = clientes.clienteid where valortotal between 200 and 270 order by nome asc;

select * from pedidos where DataPedido between "2023-09-27" and "2023-10-08" order by DataPedido asc;

select * from pedidos where ValorTotal < 45.76 limit 0,5;

select nome from clientes where nome like "%o";

select Clientes.nome,ItensPedidos.produtoid
from clientes
join ItensPedidos
on  
l;
...
select * from clientes 
left join pedidos
on pedidos.clienteid = clientes.clienteid where pedidoid is null;

select nome from clientes where nome like "a%";

select nome from clientes where nome like "%arc%";

select pedidos.pedidoID,pedidos.datapedido,pedidos.valortotal,produtos.NomeProduto,ItensPedido.Quantidade
from pedidos
inner join produtos on Pedidos.PedidoID = produtos.pedidoID; 
